...
for r,_,fl in src_fl:
	...
	for f in fl:
		if (r[:7]=="src/efi"):
			if (f[-2:]==".c"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/efi/{(r+f).replace('/','$')}.o")):
					print(f"Compiling C File (Efi): {r+f} -> build/efi/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["bash","-c",f"gcc -Isrc/kernel/include -I/usr/include/efi -I/usr/include/efi/x86_64 -I/usr/include/efi/protocol -fno-stack-protector -O3 -fpic -fshort-wchar -fno-common -mno-red-zone -DHAVE_USE_MS_ABI -Wall -Werror -c {r+f} -o build/efi/{(r+f).replace('/',chr(92)+'$')}.o"]).returncode!=0 or subprocess.run(["strip","-R",".rdata$zzz","--keep-file-symbols","--strip-debug","--strip-unneeded","--discard-locals",f"build/efi/{(r+f).replace('/','$')}.o"]).returncode!=0):
					...
					print(f"Compiling C File (Efi): {r+f} -> build/efi/{(r+f).replace('/','$')}.o (Already Exists)")
				e_fl+=[f"build/efi/{(r+f).replace('/','$')}.o"]
				cl[r+f]=n_f_h_dt[r+f]
			elif (f[-4:]==".asm"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/efi/{(r+f).replace('/','$')}.o")):
					print(f"Compiling ASM File (Efi): {r+f} -> build/efi/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["nasm",r+f,"-f","elf64","-O3","-Wall","-Werror","-o",f"build/efi/{(r+f).replace('/','$')}.o"]).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
			...
			if (f[-2:]==".c"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/kernel/{(r+f).replace('/','$')}.o")):
					...
					print(f"Compiling C File (Kernel): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o (Already Exists)")
				k_fl+=[f"build/kernel/{(r+f).replace('/','$')}.o"]
				cl[r+f]=n_f_h_dt[r+f]
			elif (f[-4:]==".asm"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/kernel/{(r+f).replace('/','$')}.o")):
					print(f"Compiling ASM File (Kernel): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["nasm",r+f,"-f","elf64","-O3","-Wall","-Werror","-o",f"build/kernel/{(r+f).replace('/','$')}.o"]+asm_d).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
					...
					print(f"Compiling ASM File (Kernel): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o (Already Exists)")
				k_fl+=[f"build/kernel/{(r+f).replace('/','$')}.o"]
				cl[r+f]=n_f_h_dt[r+f]
		elif (r[:4]=="rsrc"):
			if (f[-2:]==".c"):
			...
			if (f[-2:]==".c"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/kernel/{(r+f).replace('/','$')}.o")):
					print(f"Compiling C File (Kernel LibC): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["gcc","-mcmodel=large","-mno-red-zone","-fno-common","-m64","-Wall","-Werror","-fpic","-ffreestanding","-fno-stack-protector","-O3","-nostdinc","-nostdlib","-c",r+f,"-o",f"build/kernel/{(r+f).replace('/','$')}.o","-Isrc/kernel/include","-Isrc/libc/include","-Irsrc","-D__KERNEL__=1"]).returncode!=0 or subprocess.run(["strip","-R",".rdata$zzz","--keep-file-symbols","--strip-debug","--strip-unneeded","--discard-locals",f"build/kernel/{(r+f).replace('/','$')}.o"]).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
					...
					print(f"Compiling C File (Kernel LibC): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o (Already Exists)")
				k_fl+=[f"build/kernel/{(r+f).replace('/','$')}.o"]
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/libc/{(r+f).replace('/','$')}.o")):
					print(f"Compiling C File (LibC): {r+f} -> build/libc/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["gcc","-mcmodel=large","-mno-red-zone","-fno-common","-m64","-Wall","-Werror","-fpic","-ffreestanding","-fno-stack-protector","-O3","-nostdinc","-nostdlib","-c",r+f,"-o",f"build/libc/{(r+f).replace('/','$')}.o","-Isrc/libc/include","-Irsrc"]).returncode!=0 or subprocess.run(["strip","-R",".rdata$zzz","--keep-file-symbols","--strip-debug","--strip-unneeded","--discard-locals",f"build/libc/{(r+f).replace('/','$')}.o"]).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
					...
					print(f"Compiling C File (LibC): {r+f} -> build/libc/{(r+f).replace('/','$')}.o (Already Exists)")
				l_fl+=[f"build/libc/{(r+f).replace('/','$')}.o"]
				cl[r+f]=n_f_h_dt[r+f]
			elif (f[-4:]==".asm"):
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/kernel/{(r+f).replace('/','$')}.o")):
					print(f"Compiling ASM File (Kernel LibC): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["nasm",r+f,"-f","elf64","-O3","-Wall","-Werror","-D__KERNEL__=1","-o",f"build/kernel/{(r+f).replace('/','$')}.o"]).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
					...
					print(f"Compiling ASM File (Kernel LibC): {r+f} -> build/kernel/{(r+f).replace('/','$')}.o (Already Exists)")
				k_fl+=[f"build/kernel/{(r+f).replace('/','$')}.o"]
				if (_check_new(f_h_dt,n_f_h_dt,f_inc,r+f,f"build/libc/{(r+f).replace('/','$')}.o")):
					print(f"Compiling ASM File (LibC): {r+f} -> build/libc/{(r+f).replace('/','$')}.o")
					if (subprocess.run(["nasm",r+f,"-f","elf64","-O3","-Wall","-Werror","-o",f"build/libc/{(r+f).replace('/','$')}.o"]).returncode!=0):
						print("Writing File Hash List...")
						with open("build/__last_build_files__","w") as f:
							for k,v in cl.items():
								f.write(f"{k}{v}\n")
					...
					print(f"Compiling ASM File (LibC): {r+f} -> build/libc/{(r+f).replace('/','$')}.o (Already Exists)")
				l_fl+=[f"build/libc/{(r+f).replace('/','$')}.o"]
				cl[r+f]=n_f_h_dt[r+f]
print("Writing File Hash List...")
with open("build/__last_build_files__","w") as f:
