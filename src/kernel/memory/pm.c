...
void KERNEL_CALL pm_init(KernelArgs* ka){
	uint64_t sz=(((((ka->mmap[ka->mmap_l-1].b&0xfffffffffffffffe)+ka->mmap[ka->mmap_l-1].l-1+PAGE_4KB-1)>>PAGE_4KB_POWER_OF_2)+sizeof(PageData)*BITS_IN_BYTE-1)/(sizeof(PageData)*BITS_IN_BYTE)*sizeof(PageData)+sizeof(PageArrayData)+PAGE_4KB-1)>>PAGE_4KB_POWER_OF_2;
	...
	_pg_dt->l=((((ka->mmap[ka->mmap_l-1].b&0xfffffffffffffffe)+ka->mmap[ka->mmap_l-1].l-1+PAGE_4KB-1)>>PAGE_4KB_POWER_OF_2)+sizeof(PageData)*BITS_IN_BYTE-1)/(sizeof(PageData)*BITS_IN_BYTE);
	...
	while (1){
		...
		if (i>=(ka->mmap[j].b&0xfffffffffffffffe)+ka->mmap[j].l){
			j++;
			while (j<ka->mmap_l&&ka->mmap[j].b&1){
				j++;
			}
			if (j==ka->mmap_l){
				break;
